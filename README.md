...
- **History search**: use **Search history** in the sidebar to filter conversation titles and existing short previews, including archived sessions. Matching is literal, case-insensitive and supports Chinese text; Escape or the clear button restores the list without changing the current conversation. Workspace matches temporarily reveal their folders, then restore your expansion choices. The Claude addins include the full available metadata catalog rather than only the latest 100 sessions; ordinary custom widgets search the stubs supplied by their host. Lists initially render 100 rows per section/folder, with **Show more**, but search covers all supplied metadata before that rendering limit. Typing does not scan transcripts, generate summaries or contact a model.
- **Readable web links**: assistant Markdown, tool-result Markdown and URL arguments use underlined blue links, independently of the button `primary` theme: `#2563EB` on light themes and `#60A5FA` on dark themes. The scoped `--aui-link-color` and `--aui-link-hover-color` CSS variables can override this palette. Keyboard focus remains visible; host-page links and local file-reference controls are unchanged.
- **Confirmed file links**: inline filenames become blue, underlined links only after the backend confirms an openable target. Missing, ambiguous or unconfirmed references stay gray and noninteractive, including bare names mentioned only inside Bash commands. The addin checks explicit paths (including `~/`), project-relative paths and unique matches in its already-warm workspace index; it never scans Home or guesses shell expressions. Checks are batched and cached per widget/thread/project without reading file contents. Custom hosts using `on_open_file` can provide `file_reference_resolver` for their own navigation backend.
- **Responsive file navigation**: reference checks yield between bounded R slices (at most four paths, checking an 8 ms elapsed-work budget between calls), so a slow batch does not monopolize the event loop. A single slow filesystem call can still exceed that budget and block its slice. Previously confirmed links stay usable during revalidation, avoiding a lost click when refresh begins between pointer-down and click; a negative result removes the link. Explicit clicks focus the file and requested line even if it is already open, while automatic edit reveal remains non-disruptive. Opening feedback waits for the backend acknowledgement, reports failures, and coalesces duplicate pending clicks; legacy callbacks without this protocol retain timed feedback.
- **File-link performance comparison**: run [`benchmark_file_references.R`](tests/verify/benchmark_file_references.R) through the bounded verifier's `browser` mode with `--timeout 180`, `AUI_FILE_REFERENCE_PERF_REP=1`, then `2` and `3`, and `AUI_FILE_REFERENCE_PERF_OUT` set to an evidence directory (use a fresh runner state per repetition). It compares the same installed package with confirmation off, real project-filesystem metadata checks, and an explicitly injected 20 ms per lookup. Each repetition uses a different arm order, 240 synthetic history messages, fresh R/Chromium processes, browser input-frame timing and R heartbeat gaps. It verifies cached typing performs no new checks, reads no file contents, clears no kernel caches, and removes its uniquely owned fixture directory. Slow-query results model blocking; they are not measurements of actual storage latency.
...
- **Reading session memory**: the Orb separates the plugin's PSS/RSS guard from the whole-session cgroup. **Raw total** includes file pages; **Working set estimate** subtracts `inactive_file` and is neither process memory nor a guaranteed allocation budget. File pages include shmem and are not all immediately reclaimable. Limit/OOM counters show both cumulative totals and changes between actual background samples; PSI shows 10-second moving averages of memory-stalled time. Missing metrics stay **Unavailable**, not zero or Unlimited. Opening or Refresh reads one frozen background snapshot; it does not clear caches, change guard policy, or enable additional polling.
