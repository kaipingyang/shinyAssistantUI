# shinyAssistantUI 0.5.7.9004

- **顶部 Question 与历史阅读位置一致**：滚到第一条消息上方的留白时，当前问题条显示首个问题，不再回退到最后一轮；重新打开历史也采用同一规则。保留虚拟化和缓存高度定位，不增加逐消息 DOM 扫描或输入时重算。

# shinyAssistantUI 0.5.7.9003

- **历史会话冷连接不再阻塞其它会话**：前台连接改为等待 SDK 的单个异步初始化结果；另一个历史会话首次启动 CLI 时，已有会话仍可持续更新和处理 Shiny 事件。保留每线程 FIFO、全局并发上限、单 reader、审批和原性能边界。连接中 Stop 会取消自己的初始化，不发送用户消息，也不因取消而回退到全新会话；正常恢复失败的兼容回退及严格恢复限制不变。需要 ClaudeAgentSDK `>= 0.2.5.9001`，更新后须重启已有 Job。
