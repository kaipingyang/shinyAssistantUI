...
separate R **worker process** pinned to a library with a compatible
(new) `curl` (e.g. `Rlibs/codeagent/R-4.4`). The MAIN Shiny process
**never loads codeagent / ellmer / curl** — so it is safe inside a
session whose default library has an old, incompatible `curl` (the R
...
## Arguments
  ...
  - libpath:
    ...
    Library path the worker prepends to `.libPaths()` — the isolated
    codeagent lib (new curl), e.g. the shared `Rlibs/codeagent/R-4.4`.
...
in-process (e.g. Posit Workbench / ERP with a legacy system stack). On a
machine whose whole environment already has a new `curl`, the lighter
in-process `make_codeagent_handler()` works too.
