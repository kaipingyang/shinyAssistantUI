...
click_element <- function(selector, index = "last") {
  ...
  if (is.null(point) || !isTRUE(point$visible)) return(FALSE)
...
send_message <- function(root, text) {
  ...
  if (!click_element(selector)) return(FALSE)
  Sys.sleep(0.15)
...
check("fixture uses exact HOME-installed package", isTRUE(value(sprintf(
  "document.querySelector('#package-meta').dataset.packagePath===%s && document.querySelector('#package-meta').dataset.packageVersion==='0.5.7.9000'",
...
check("R received live attachment bytes on first send",
...
check("reload replays identical attachment fingerprint",
...
check("positive feedback clicked in viewport",
...
check("negative feedback clicked in viewport",
...
check("no browser runtime exceptions", length(runtime_exceptions) == 0L,
      paste(head(runtime_exceptions, 3L), collapse = " | "))
check("no browser network failures", length(network_failures) == 0L,
