...
}, args = list(project = project, port = port), stdout = "/tmp/aui-edit.out", stderr = "/tmp/aui-edit.err")
on.exit(try(app$kill(), silent = TRUE), add = TRUE)
for (i in seq_len(100)) { if (!app$is_alive()) break; if (file.exists("/tmp/aui-edit.err") && any(grepl("Listening on", readLines("/tmp/aui-edit.err", warn = FALSE)))) break; Sys.sleep(0.25) }
if (!app$is_alive()) { cat(tail(readLines("/tmp/aui-edit.err", warn = FALSE), 20), sep = "\n"); stop("boot failed") }
...
# 1) 首发消息
click_xy(".aui-lexical-input[contenteditable='true']"); Sys.sleep(0.35)
browser$Input$insertText(text = "first"); Sys.sleep(0.25)
...
hover_msg <- function() {
  ...
  if (is.null(j)) return(FALSE); p <- fromJSON(j)
  browser$Input$dispatchMouseEvent(type = "mouseMoved", x = p$x, y = p$y); TRUE
}
hover_msg(); Sys.sleep(0.3)
...
# 3) 改文本:聚焦 → 全选 → 输入新文本
click_xy(".aui-edit-composer-input"); Sys.sleep(0.2)
press_key("a", "KeyA", 65L, mods = 2L)  # Ctrl+A 全选
Sys.sleep(0.1)
browser$Input$insertText(text = "second"); Sys.sleep(0.25)
...
chk("edit->Update RE-TRIGGERS handler (echo: second)", wait_for("document.body.innerText.includes('echo: second')", 12),
