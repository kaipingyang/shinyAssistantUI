...
PROJ<-"/usrfiles/shared-projects/users/kaiping_yang/shinyAssistantUI"; PORT<-9843L
...
has_gui   <- isTRUE(ev("!!document.querySelector('[data-slot=gui-card]') && !!document.querySelector('[data-slot=gui-heading]') && (document.querySelector('[data-slot=gui-stat]')?.textContent||'').includes('123.4')"))
...
cat(sprintf("[%s] A1 generative-ui primitive: Card>Stack>[Heading,Stat] rendered\n", if(has_gui)"PASS" else "FAIL"))
