...
main <- function() {
  ...
  stopifnot(
    identical(normalizePath(find.package("shinyAssistantUI")), file.path(home_lib, "shinyAssistantUI")),
  ...
  proc <- callr::r_bg(
    function(app_file, port, home_lib) {
  ...
  wait_for <- function(js, timeout = 20, interval = 0.2) {
    ...
    repeat {
      ...
      if (!inherits(value, "try-error") && isTRUE(value)) {
        return(TRUE)
      }
  ...
  assert <- function(ok, label) {
    if (!isTRUE(ok)) {
      cat("ASK_STATE ", ev(
        "JSON.stringify((()=>{const form=document.querySelector('[data-slot=ask-user-question]');return {form:!!form,selected:form?.querySelector('[data-ask-option=\"Yes\"] input')?.checked,disabled:form?.querySelector('[data-ask-submit]')?.disabled,text:form?.closest('.aui-shiny-tool')?.innerText.slice(-300),events:window.__askEvents||[]}})())"
      ), "\n")
      if (length(c(console_errors, runtime_errors))) {
        cat("PAGE_ERRORS ", paste(c(console_errors, runtime_errors), collapse = " | "), "\n")
      }
      stop("FAIL: ", label)
    }
    cat("PASS:", label, "\n")
  ...
  activate_control <- function(selector) {
    ...
    assert(isTRUE(ev(sprintf(
      "(()=>{const e=document.querySelector(%s);if(!e)return false;e.scrollIntoView({block:'center',behavior:'instant'});return true})()",
      encoded
    ))), paste("question control exists", selector))
    assert(wait_for(sprintf(
      "(()=>{const r=document.querySelector(%s).getBoundingClientRect();return r.width>0&&r.height>0&&r.top>=0&&r.left>=0&&r.bottom<=innerHeight+1&&r.right<=innerWidth+1})()",
      encoded
    )), "question control is inside viewport")
    assert(isTRUE(ev(sprintf(
      "(()=>{const e=document.querySelector(%s);e.focus({preventScroll:true});return document.activeElement===e&&!e.disabled})()",
      encoded
    ))), "enabled question control receives keyboard focus")
    for (type in c("keyDown", "keyUp")) {
      b$Input$dispatchKeyEvent(
        type = type, key = " ", code = "Space", windowsVirtualKeyCode = 32L
      )
    }
  }
  trigger_js <- function(text) {
    sprintf(
  ...
  request_count <- function(id = "lazy_request_count") {
    as.integer(ev(sprintf(
  ...
  assert(
    identical(ev("document.getElementById('installed-version').textContent.trim()"), expected_version),
    paste("Home installed package version", expected_version)
  )
  ...
  assert(
    identical(ev(sprintf("(%s).getAttribute('aria-expanded')", live_trigger)), "false"),
  ...
  assert(
    wait_for(sprintf("(%s).getAttribute('aria-expanded') === 'false'", ask_trigger)),
    "pending AskUserQuestion card is collapsed"
  )
  assert(
    wait_for("!!document.querySelector('[data-slot=ask-user-question]')"),
  ...
  assert(
    wait_for("!!document.querySelector('[data-lazy-tool-result]') && document.body.innerText.includes('LAZY-LIVE-LINE-00001')"),
    "opening loads first bounded chunk"
  )
  assert(
    wait_for("Number((document.getElementById('live_request_count')?.textContent||'0').trim()) >= 1"),
  ...
  assert(
    wait_for("Number((document.getElementById('live_request_count')?.textContent||'0').trim()) >= 2"),
    "near-bottom scroll requested the next chunk"
  )
  assert(
    wait_for(sprintf("document.querySelector('[data-lazy-tool-result]').innerText.length > %d", first_len)),
    "next chunk extends one continuous scroll view"
  )
  assert(
    !isTRUE(ev("Array.from(document.querySelectorAll('[data-lazy-tool-result] button')).some(b=>/next|previous|page/i.test(b.textContent))")),
  ...
  for (expected_requests in 3:10) {
    assert(
      wait_for("!document.querySelector('[data-lazy-result-loading]')"),
      sprintf("chunk %d settled", expected_requests - 1L)
    )
    ev("(function(){const e=document.querySelector('[data-slot=tool-result-scroll]');e.scrollTop=e.scrollHeight;e.dispatchEvent(new Event('scroll',{bubbles:true}));return true})()")
    assert(
      wait_for(sprintf("Number((document.getElementById('live_request_count')?.textContent||'0').trim()) >= %d", expected_requests)),
      sprintf("continuous scroll requested chunk %d", expected_requests)
    )
  }
  assert(
    wait_for("!!document.querySelector('[data-lazy-result-spacer]')"),
    "client evicts old chunks after its retained window"
  )
  assert(
    isTRUE(ev("parseFloat(document.querySelector('[data-lazy-result-spacer]').style.height) > 0")),
    "eviction preserves released logical height"
  )
  assert(
    isTRUE(ev("(function(){const e=document.querySelector('[data-slot=tool-result-scroll]');return e.scrollHeight-e.scrollTop-e.clientHeight<=130})()")),
  ...
  ev(paste0(
    "window.__askEvents=[];['pointerdown','pointerup','click','change'].forEach(type=>",
    "document.addEventListener(type,event=>{const e=event.target;if(!(e instanceof Element))return;",
    "window.__askEvents.push({type,tag:e.tagName,slot:e.getAttribute('data-slot'),",
    "text:e.textContent.slice(0,60),checked:e.checked,x:event.clientX,y:event.clientY});",
    "window.__askEvents=window.__askEvents.slice(-12)},true));true"
  ))
  ev(sprintf("(%s).click(); true", ask_trigger))
  assert(
    wait_for(sprintf("(%s).getAttribute('aria-expanded') === 'true'", ask_trigger)),
    "pending AskUserQuestion card can be manually opened"
  )
  activate_control('[data-ask-option="Yes"] input')
  assert(wait_for(
    "document.querySelector('[data-ask-option=\"Yes\"] input')?.checked && document.querySelector('[data-ask-submit]')?.disabled===false"
  ), "selected answer enables submission")
  activate_control("[data-ask-submit]")
  Sys.sleep(0.2)
  cat("ASK_AFTER_SUBMIT ", ev(sprintf(
    paste0(
      "JSON.stringify((()=>{const t=%s,c=t.closest('.aui-shiny-tool');return {",
      "open:t.getAttribute('aria-expanded'),form:!!c.querySelector('[data-slot=ask-user-question]'),",
      "submitDisabled:c.querySelector('[data-ask-submit]')?.disabled,",
      "text:c.innerText.slice(-300)}})())"
    ),
    ask_trigger
  )), "\n")
  if (length(c(console_errors, runtime_errors))) {
  ...
  assert(
    identical(ev(sprintf("(%s).getAttribute('aria-expanded')", history_trigger)), "false"),
  ...
  assert(
    !isTRUE(ev("document.body.innerText.includes('LAZY-HISTORY-LINE-00001')")),
    "historical collapsed body is absent"
  )
  ev(sprintf("(%s).click(); true", history_trigger))
  assert(
    wait_for("document.body.innerText.includes('LAZY-HISTORY-LINE-00001')"),
    "historical tool loads on demand"
  )
  assert(
    wait_for("Number((document.getElementById('history_request_count')?.textContent||'0').trim()) >= 1"),
