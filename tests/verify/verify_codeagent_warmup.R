...
PROJ<-"/usrfiles/shared-projects/users/kaiping_yang/shinyAssistantUI"; PORT<-9869L
...
warmed_seen<-FALSE; for(i in 1:24){if(isTRUE(ev("!!document.querySelector('[data-slot=aui_warming]')"))){warmed_seen<-TRUE;break}; Sys.sleep(0.25)}
cat(sprintf("[%s] warming indicator shown during background prewarm\n", if(warmed_seen)"PASS" else "FAIL"))
