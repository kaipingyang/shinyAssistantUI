...
errs<-c(); failures<-character(); b<-chromote::ChromoteSession$new(); b$Runtime$enable()
...
b$Runtime$exceptionThrown(callback_=function(m) errs<<-c(errs, m$exceptionDetails$exception$description %||% "Runtime.exceptionThrown"))
...
# click the newest currently-actionable approval button with exact label
click_btn<-function(label) ev(sprintf("(function(){var bs=[].slice.call(document.querySelectorAll('.aui-shiny-approval button')).filter(function(x){return (x.innerText||'').trim()===%s && !x.disabled});var b=bs[bs.length-1];if(b){b.click();return true}return false})()", paste0("'",label,"'")))
...
ready2 <- wait_for("!!document.querySelector('.aui-composer-send') && !document.querySelector('.aui-composer-cancel')", 30)
if(isTRUE(ready2)) send("Now use run_r to print the text DENYME. One tool call.")
a2 <- if(isTRUE(ready2)) wait_for("(function(){return [].slice.call(document.querySelectorAll('.aui-shiny-approval button')).some(function(x){return (x.innerText||'').trim()==='Deny'&&!x.disabled})})()", 75) else FALSE
...
if(!isTRUE(a1) || !isTRUE(inview) || !isTRUE(ok1) || !isTRUE(appr) || !isTRUE(exec)) failures<-c(failures,"approve round")
if(!isTRUE(a2) || !isTRUE(ok2) || !isTRUE(den)) failures<-c(failures,"deny round")

cat(sprintf("[%s] no console errors (%d)\n", if(length(errs)==0)"PASS" else "FAIL", length(errs)))
if(length(errs)) { cat("  errs:", paste(utils::head(errs,3),collapse=" | "),"\n"); failures<-c(failures,"console errors") }
if(!p$is_alive()) { cat("  (app process died mid-run!)\n"); failures<-c(failures,"app died") }
if(length(failures)) {
  cat("  body:", substr(ev("document.body.innerText"), 1, 2000), "\n")
  if(file.exists("/tmp/caap.e")) {
    cat("  backend stderr:\n", paste(tail(readLines("/tmp/caap.e", warn=FALSE), 40), collapse="\n"), "\n")
  }
}
b$close(); p$kill()
if(length(failures)) stop("CODEAGENT_APPROVAL failed: ",paste(failures,collapse=", "))
