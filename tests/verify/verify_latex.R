...
app <- callr::r_bg(function(project, port) {
  ...
  shiny::runApp("tests/verify/latex_app.R", host = "127.0.0.1", port = port, launch.browser = FALSE)
}, args = list(project = project, port = port), stdout = "/tmp/aui-latex.out", stderr = "/tmp/aui-latex.err")
on.exit(try(app$kill(), silent = TRUE), add = TRUE)
for (i in seq_len(100)) { if (!app$is_alive()) break; if (file.exists("/tmp/aui-latex.err") && any(grepl("Listening on", readLines("/tmp/aui-latex.err", warn = FALSE)))) break; Sys.sleep(0.25) }
if (!app$is_alive()) { cat(tail(readLines("/tmp/aui-latex.err", warn = FALSE), 20), sep = "\n"); stop("boot failed") }
...
chk("KaTeX rendered inline math (.katex present)", wait_for("!!document.querySelector('.katex')", 12))
chk("both formulas rendered (inline + block, .katex count >= 2)",
    wait_for("document.querySelectorAll('.katex').length >= 2", 8),
    value("document.querySelectorAll('.katex').length+''"))
chk("block math renders as display (.katex-display)",
    isTRUE(value("!!document.querySelector('.katex-display')")),
    value("(document.querySelector('.katex-display')?.textContent||'').slice(0,30)"))
